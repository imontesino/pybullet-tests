...
parser.add_argument('--step_duration', type=float,default=5, help='duration of a step in seconds')
...
l_hip_correction   = trajectory_pol( 0,   0,      0,          max_offset/2,             0,                         0,  int(n_timesteps/2))
...
for t in range(int(n_timesteps/2)):
    ...
    targetPos[name2id[b'l_hip_roll']] = leftLegAngles[-2] + l_hip_correction(t)
...
leftLegAngles[-2] = leftLegAngles[-2] + l_hip_correction(t)
...
for i in range(n_steps):
    ...
    l_hip_correction   = trajectory_pol( 0,   0,      0,           -max_offset,              0,                         0,  int(n_timesteps/2))
    ...
    r_hip_correction   = trajectory_pol( 0,   0,      0,          -max_offset/2,              0,                         0,  int(n_timesteps/2))
    ...
    for t in range(int(n_timesteps/2)):
        ...
        targetPos[name2id[b'r_hip_roll']] = rightLegAngles[-2] + r_hip_correction(t)
    ...
    r_hip_correction   = trajectory_pol( -max_offset/2,   0,      0,            -max_offset,              0,                         0,  int(n_timesteps/2))
    ...
    for t in range(n_timesteps):
        ...
        targetPosOffset[name2id[b'l_hip_roll']] = -max_offset
        targetPosOffset[name2id[b'r_hip_roll']] =  max_offset
